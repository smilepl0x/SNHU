...
int partition(vector<Bid>& bids, int begin, int end) {
    ...
    while (!done) {
        ...
        // keep decrementing high as long as it is greater than pivot
...
void quickSort(vector<Bid>& bids, int begin, int end) {
    ...
    // recursively call quicksort using midpoint values (begin to mid)
    quickSort(bids, begin, mid);

    // recursively call quicksort using midpoint values (mid +1 to end)
...
void selectionSort(vector<Bid>& bids) {
    // index for current minumum
    int min;

    // pos is the position within bids that marks sorted/unsorted
