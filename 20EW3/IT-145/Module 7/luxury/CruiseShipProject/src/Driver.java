...
public class Driver {
    ...
    public static void main(String[] args) {
        ...
        // input variable & scanner
        Scanner scnr = new Scanner(System.in);
        String userInput = "";
        
        // show menu until exit
        while (!userInput.contentEquals("1") || !userInput.contentEquals("2") || !userInput.contentEquals("3") || !userInput.contentEquals("4")
        		|| !userInput.contentEquals("5") || !userInput.contentEquals("6") || !userInput.contentEquals("A") || !userInput.contentEquals("B")
        		|| !userInput.contentEquals("C") || !userInput.contentEquals("D") || !userInput.contentEquals("E") || !userInput.contentEquals("F")
        		|| !userInput.contentEquals("x")) {
        	displayMenu();
    ...
    public static void printShipList(String listType) {
        ...
        if (listType == "name") {
            ...
            System.out.println("Number of Ships in Service: " + shipsInService);
    ...
    public static void addShip() {
    	...
    	// ensure ship does not already exist
    	for (Ship eachShip: shipList) {
    	...
    	// enter remaining info
    	System.out.println("Enter number of balcony rooms: ");
    	int balconyRooms = Integer.parseInt(newShipInput.nextLine());
    	
    	System.out.println("Enter number of ocean view rooms: ");
    	int oceanViewRooms = Integer.parseInt(newShipInput.nextLine());
    	
    	System.out.println("Enter number of suite rooms: ");
    	int suiteRooms = Integer.parseInt(newShipInput.nextLine());
    	
    	System.out.println("Enter number of interior rooms: ");
    	int interiorRooms = Integer.parseInt(newShipInput.nextLine());
    	
    	// enter service status
    	boolean serviceStatus;
    	String inputServiceStatus = "";
    	while (!inputServiceStatus.equalsIgnoreCase("y") || !inputServiceStatus.equalsIgnoreCase("n")) { // input has to be y or n
    		System.out.println("Is this ship in service? (y/n)");
    		inputServiceStatus = newShipInput.nextLine();
    		if (inputServiceStatus.equalsIgnoreCase("y")) {
    			serviceStatus = true;
    			Ship newShip = new Ship(newShipName, balconyRooms, oceanViewRooms, suiteRooms, interiorRooms, serviceStatus);
    			shipList.add(newShip); // adds ship in service
    			break;
    		}
    		else if (inputServiceStatus.equalsIgnoreCase("n")) {
    			serviceStatus = false;
    			Ship newShip = new Ship(newShipName, balconyRooms, oceanViewRooms, suiteRooms, interiorRooms, serviceStatus);
    			shipList.add(newShip); // adds ship not in service
    			break;
    		}
    		else {
    ...
    public static void addCruise() {
    	...
    	// ensure cruise does not already exist
    	for (Cruise eachCruise: cruiseList) {
    	...
    	System.out.println("Enter ship name for " + newCruiseName + ": ");
    	String newCruiseShipName = newCruiseInput.nextLine();
    	boolean shipExists = false;
    	
    	// ensure the ship is in the system and in service
    	for (Ship eachShip: shipList) {
    	...
    	// everything should pass at this point. add to arrayList
